...
function Install-Software {
    ...
    foreach ($software in $softwareFiles) {
        ...
        if ($software.Extension -eq ".exe") {
            # Start the installation process for .exe files
            Start-Process -FilePath $software.FullName -ArgumentList '/silent' -Wait -NoNewWindow
        } elseif ($software.Extension -eq ".msi") {
            # Start the installation process for .msi files
